...
/** ✅ Response dari GET /api/admin/content?key=... (atau fallback) */
type ApiOk = {
  from: "db" | "fallback";
  key: string;
  title: string;
...
type ApiErr = { error: string; extra?: any };
type ApiGetResponse = ApiOk | ApiErr;
...
function isApiOk(x: any): x is ApiOk {
  return (
    !!x &&
    typeof x === "object" &&
    (x.from === "db" || x.from === "fallback") &&
    typeof x.key === "string" &&
    typeof x.title === "string" &&
    x.content &&
    typeof x.content === "object" &&
    Array.isArray(x.content.items)
  );
}
...
export default function AdminContentPage() {
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      ...
      // HTTP error
      if (!res.ok) {
        const errMsg = isApiErr(json) ? json.error : `HTTP ${res.status}`;
      ...
      // sukses
      if (isApiOk(json)) {
        const title = (json.title || defaultTitle).trim() || defaultTitle;
        setBucketTitle(title);
        setBucket({
          title,
          items: Array.isArray(json.content.items) ? json.content.items : fallbackItems,
        });
      } else {
        // response aneh / kosong -> fallback template
  ...
  const addCategory = () => {
    ...
    const exists = categories.some((c) => c.id === id || c.title.toLowerCase() === name.toLowerCase());
          ...
          {categories.length === 0 ? (
            ...
            <div className="space-y-2">
              {categories.map((c) => (
                ...
                >
                  <button type="button" className="text-left flex-1" onClick={() => setCategoryId(c.id)}>
                        ...
                        {cat.children.map((it) => (
                          ...
                          >
                            <div className="min-w-[260px]">
                              ...
                              {it.desc ? <div className="text-xs text-slate-600 mt-1">{it.desc}</div> : null}
                            </div>
