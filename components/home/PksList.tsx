  ...
  return (
    <section className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white/90 dark:bg-slate-900/70 p-5">
      ...
      <p className="text-sm text-slate-500 mt-1">
        Klik salah satu pabrik untuk melihat detail profilnya.
      </p>

      {loading ? (
        <div className="mt-6 text-center text-slate-500">Memuat data PKS...</div>
      ) : (
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 mt-4">
          {safeList.map((pks: any) => (
            <motion.article
              key={pks.id}
              whileHover={{ y: -2 }}
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") onSelect(pks.id);
              }}
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  {/* slug sengaja tidak ditampilkan */}
                </div>
